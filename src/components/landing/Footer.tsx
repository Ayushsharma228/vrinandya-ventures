import { C } from "./constants";

export function Footer() {
  return (
    <footer style={{ background: C.navy, borderTop: `1px solid ${C.border}` }}>
      <div className="max-w-[1200px] mx-auto px-6 py-5 flex flex-wrap items-center gap-y-3 justify-between">

        <span className="font-black text-sm" style={{ color: C.heading, fontFamily: "var(--font-space)" }}>
          AXQEN
        </span>

        <div className="flex flex-col items-center gap-0.5">
          <p className="text-xs" style={{ color: C.muted }}>
            © {new Date().getFullYear()} AXQEN. India&apos;s COD Dropshipping Platform.
          </p>
          <p className="text-xs" style={{ color: C.muted }}>
            Product by{" "}
            <a href="/company" className="font-semibold hover:opacity-70 transition-opacity" style={{ color: C.body }}>
              Vrinandya Ventures Pvt. Ltd.
            </a>
          </p>
        </div>

        <div className="flex items-center gap-4 text-sm" style={{ color: C.body }}>
          <a href="/company" className="hover:opacity-70 transition-opacity text-xs">Company</a>
          <a href="/login"   className="hover:opacity-70 transition-opacity">Login</a>
          <a href="/signup"  className="hover:opacity-70 transition-opacity">Register</a>
        </div>

      </div>
    </footer>
  );
}
