  ...
  const fetchPOs = useCallback(async () => {
    ...
    try {
      const url = statusFilter
        ? `/api/supplier/purchase-orders?status=${statusFilter}`
  ...
  return (
    <div className="min-h-screen" style={{ background: "var(--bg-page)" }}>
      <PageHero
        ...
        subtitle="All purchase orders assigned to you by AXQEN"
        cards={
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {[
              { label: "Total POs",  value: pos.length },
              { label: "Pending",    value: pos.filter((p) => p.status === "SENT").length },
              { label: "Active",     value: pos.filter((p) => ["ACCEPTED","PROCESSING","PACKED"].includes(p.status)).length },
                  ...
                  {pos.map((po) => {
                    ...
                    return (
                      <tr key={po.id} className="hover:bg-gray-50/50 transition-colors">
                        ...
                        <td className="px-4 py-3.5 text-sm" style={{ color: "var(--text-900)" }}>
                          {po.order.customerName ?? "—"}
                        </td>
