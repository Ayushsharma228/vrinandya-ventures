...
import { User, Mail, Phone, Building2, FileText, CreditCard, Lock, Save, CheckCircle2 } from "lucide-react";
...
export default function SellerProfilePage() {
  ...
  return (
    <div className="min-h-screen" style={{ background: "var(--bg-page)" }}>
      <PageHero
        title="My Profile"
        subtitle="Manage your personal, business and bank details"
      />
          ...
          <div className="space-y-4">
            ...
            <div className="card p-5 flex flex-col items-center text-center">
              ...
              <span className="mt-2 px-3 py-1 rounded-full text-xs font-semibold"
                style={{ background: "#F0FDF4", color: "var(--green-500)" }}>
                Seller
              </span>
            </div>
          ...
          <div className="col-span-3 card p-6">
            {activeTab === "personal" && (
              <div className="space-y-5">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <Field icon={User} label="Full Name" value={personal.name}
                    onChange={v => setPersonal(p => ({ ...p, name: v }))} />
                  <Field icon={Mail} label="Email Address" value={personal.email}
                    onChange={v => setPersonal(p => ({ ...p, email: v }))} type="email" disabled />
                  <Field icon={Phone} label="Phone Number" value={personal.phone}
                    onChange={v => setPersonal(p => ({ ...p, phone: v }))} type="tel" />
                </div>
              </div>
            )}
            ...
            {activeTab === "business" && (
              <div className="space-y-5">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <Field icon={Building2} label="Brand / Business Name" value={business.brandName}
                    onChange={v => setBusiness(p => ({ ...p, brandName: v }))} />
                  <Field icon={FileText} label="GST Number (optional)" value={business.gst}
                    onChange={v => setBusiness(p => ({ ...p, gst: v }))} />
                </div>
              </div>
            )}
            ...
            {activeTab === "bank" && (
              <div className="space-y-5">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <Field icon={User} label="Account Holder Name" value={bank.accountHolder}
                    onChange={v => setBank(p => ({ ...p, accountHolder: v }))} />
                  <Field icon={CreditCard} label="Account Number" value={bank.accountNumber}
                    onChange={v => setBank(p => ({ ...p, accountNumber: v }))} />
                  <Field icon={Building2} label="IFSC Code" value={bank.ifsc}
                    onChange={v => setBank(p => ({ ...p, ifsc: v }))} />
                  <Field icon={Building2} label="Bank Name" value={bank.bankName}
            ...
            {activeTab === "password" && (
              <div className="space-y-5">
                ...
                <div className="grid grid-cols-1 gap-4 max-w-sm">
                  <Field icon={Lock} label="Current Password" value={passwords.current}
                    onChange={v => setPasswords(p => ({ ...p, current: v }))} type="password" />
                  <Field icon={Lock} label="New Password" value={passwords.newPass}
                    onChange={v => setPasswords(p => ({ ...p, newPass: v }))} type="password" />
                  <Field icon={Lock} label="Confirm New Password" value={passwords.confirm}
                    onChange={v => setPasswords(p => ({ ...p, confirm: v }))} type="password" />
                </div>
              </div>
            )}
