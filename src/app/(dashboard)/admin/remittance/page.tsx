...
import { IndianRupee, CheckSquare, Square, Calculator, Send } from "lucide-react";
...
export default function AdminRemittancePage() {
  ...
  const fetchOrders = useCallback(async () => {
    ...
    // Init charge rows from DB saved values
  ...
  function toggleAll() {
    const allSelected = orders.every((o) => charges[o.id]?.include);
    setCharges((p) => {
      const next = { ...p };
  ...
  return (
    <div className="p-6 space-y-5">
      ...
      {success && (
        <div className={`rounded-xl p-4 text-sm font-semibold border ${totalNet >= 0 ? "bg-green-50 border-green-200 text-green-700" : "bg-red-50 border-red-200 text-red-700"}`}>
          Remittance created! Net amount: {fmt(success.total)} {totalNet >= 0 ? "credited" : "debited"} to seller wallet.
        </div>
      )}
            ...
            <>
                ...
                <div className="px-5 py-3 border-b border-gray-100 flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="font-semibold text-sm text-gray-900">
                      Orders Pending Remittance ({orders.length})
                    </span>
                  </div>
              ...
              <div className="bg-white border border-gray-200 rounded-xl p-5 space-y-4">
                ...
                <button
                  ...
                  disabled={submitting || selectedOrders.length === 0 || !remittanceDate}
