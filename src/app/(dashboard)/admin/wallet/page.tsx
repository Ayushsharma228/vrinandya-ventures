...
import { IndianRupee, Plus, Trash2, ChevronDown, ChevronUp } from "lucide-react";

interface Seller { id: string; name: string | null; email: string; balance: number; }
interface Transaction { id: string; type: string; amount: number; note: string | null; orderId: string | null; createdAt: string; }
...
export default function AdminWalletPage() {
  ...
  const [form, setForm] = useState({ type: "CREDIT", amount: "", note: "", orderId: "" });
  ...
  async function handleAdd() {
    ...
    setForm({ type: "CREDIT", amount: "", note: "", orderId: "" });
                    ...
                    {transactions.map((t) => (
                      <div key={t.id} className="flex items-center justify-between px-4 py-3 hover:bg-gray-50">
                        <div className="flex items-center gap-3">
                          <span className={`w-2 h-2 rounded-full ${t.type === "CREDIT" ? "bg-green-500" : "bg-red-500"}`} />
                          <div>
                            ...
                            <p className="text-xs text-gray-400">{new Date(t.createdAt).toLocaleDateString("en-IN", { day: "2-digit", month: "short", year: "numeric" })}</p>
                          </div>
                        </div>
                        <div className="flex items-center gap-3">
