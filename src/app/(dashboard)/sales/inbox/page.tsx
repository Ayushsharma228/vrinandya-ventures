...
interface Conversation {
  ...
  lead: { id: string; name: string; phone: string; stage: string; city: string | null } | null;
  messages: Message[];
}
      ...
      <div className="px-4 md:px-8 pt-6 pb-4" style={{ background: "var(--bg-card)", borderBottom: "1px solid var(--border)" }}>
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs" style={{ color: "var(--text-400)" }}>Your assigned lead conversations</p>
          </div>
          <div className="ml-auto flex items-center gap-2">
            ...
            <Link href="/sales/leads" className="text-xs px-3 py-1.5 rounded-lg font-medium" style={{ background: "var(--bg-muted)", color: "var(--text-600)" }}>
              ← My Leads
            </Link>
          </div>
        </div>
        ...
        {!activeId ? (
          ...
          <div style={{ flex: 1, display: "flex", flexDirection: "column", overflow: "hidden" }}>
            ...
            <div className="px-5 py-3 flex items-center gap-3" style={{ borderBottom: "1px solid var(--border)", background: "var(--bg-card)" }}>
              ...
              {activeConv?.lead?.id && (
                <Link href={`/sales/leads/${activeConv.lead.id}`}
            ...
            <div className="px-4 py-3" style={{ borderTop: "1px solid var(--border)", background: "var(--bg-card)" }}>
              <div className="flex gap-2 items-end">
                <textarea
                  ...
                  style={{ background: "var(--bg-muted)", border: "1px solid var(--border)", color: "var(--text-900)", outline: "none" }}
                />
